...
resource "aws_iam_role" "eks-cluster-role" {
  ...
  tags               = tomap(merge(map("Name", "${local.env}-${local.project}-eks-cluster-role"), map("ResourceType", "IAM"), local.common_tags))
}
...
resource "aws_iam_role" "eks-worker-role" {
  ...
  tags               = tomap(merge(map("Name", "${local.env}-${local.project}-eks-worker-role"), map("ResourceType", "IAM"), local.common_tags))
}
